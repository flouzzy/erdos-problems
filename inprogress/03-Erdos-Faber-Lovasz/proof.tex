...
Un hypergraphe $H = (V_H, E_H)$ est constitué d'un ensemble fini de sommets $V_H$ et d'une famille d'arêtes $E_H$ (sous-ensembles de $V_H$). Il est dit \emph{linéaire} si pour toutes arêtes distinctes $e_1, e_2 \in E_H$, $|e_1 \cap e_2| \le 1$.
\end{definition}
...
\begin{lemma}
La conjecture d'Erdős-Faber-Lovász sur le système de cliques $\mathcal{C}$ est équivalente au problème suivant : pour tout hypergraphe linéaire $H = (V_H, E_H)$ avec $|V_H| = k$, la coloration des arêtes $E_H$ telle que des arêtes incidentes ont des couleurs différentes nécessite au plus $k$ couleurs.
\end{lemma}
\begin{proof}
Construisons l'hypergraphe dual $H = (V_H, E_H)$ de notre graphe de cliques $G$.
Posons l'ensemble des sommets $V_H$ comme étant l'ensemble des indices des cliques : $V_H = \{1, 2, \dots, k\}$. Ainsi, $|V_H| = k$.
Pour chaque sommet $v \in V$ du graphe original $G$, on associe une hyperarête $e_v \in E_H$ définie par :
$$ e_v = \{ i \in \{1, \dots, k\} \mid v \in C_i \} $$
Deux sommets $u, v \in V$ sont reliés par une arête dans $G$ si et seulement si ils appartiennent à une même clique $C_i$, c'est-à-dire si et seulement si $i \in e_u \cap e_v$, ce qui équivaut à $e_u \cap e_v \neq \emptyset$.
Colorer les sommets de $G$ de manière propre revient donc exactement à attribuer à chaque hyperarête $e_v \in E_H$ une couleur de sorte que si $e_u \cap e_v \neq \emptyset$, alors les hyperarêtes reçoivent des couleurs distinctes.

Vérifions que $H$ est linéaire. Soient deux hyperarêtes distinctes $e_u, e_v \in E_H$. L'intersection $e_u \cap e_v$ correspond aux cliques contenant à la fois $u$ et $v$. Par définition de $\mathcal{C}$, deux cliques distinctes partagent au plus un sommet. Réciproquement, deux sommets distincts $u, v$ ne peuvent appartenir conjointement à plus d'une clique (sinon ces deux cliques partageraient au moins deux sommets, contredisant l'hypothèse de $\mathcal{C}$). Ainsi, $|e_u \cap e_v| \le 1$. $H$ est donc linéaire.

La conjecture EFL est donc rigoureusement transformée en l'énoncé de la borne sur l'indice chromatique fort des hypergraphes linéaires à $k$ sommets.
\end{proof}

\subsection{Lemme 2 : Borne maximale sur le degré du graphe dual}
\begin{lemma}
Dans l'hypergraphe dual $H = (V_H, E_H)$, le degré de chaque sommet $i \in V_H$, noté $d_H(i)$, vérifie $d_H(i) \le k$.
\end{lemma}
\begin{proof}
...
Le degré $d_H(i)$ est le nombre d'hyperarêtes dans $H$ qui contiennent le sommet $i$.
Une hyperarête $e_v$ contient $i$ si et seulement si le sommet correspondant $v$ dans le système original appartient à $C_i$.
...
\end{lemma}
\begin{proof}
Nous raisonnons sur l'hypergraphe dual $H$. Nous devons colorer de manière propre un ensemble d'hyperarêtes $E_H$.
Une coloration d'hyperarêtes est propre si des hyperarêtes sécantes reçoivent des couleurs différentes.
Pour une hyperarête $e_v \in E_H$ arbitraire, considérons le nombre d'autres hyperarêtes $e_u$ avec lesquelles elle a une intersection non vide.
Soit $e_v = \{i_1, i_2, \dots, i_m\}$. L'hyperarête $e_v$ relie $m$ sommets dans $V_H$. Remarquons que $m \le k$ car $V_H$ a une taille de $k$.
Pour chaque $j \in e_v$, le sommet $j \in V_H$ est contenu dans au plus $k$ hyperarêtes (Lemme 2).
En excluant $e_v$ elle-même, le sommet $j$ connecte $e_v$ à au plus $k-1$ autres hyperarêtes.
Étant donné que $H$ est linéaire, pour deux sommets distincts $j, j' \in e_v$, les ensembles d'hyperarêtes (autres que $e_v$) contenant $j$ et $j'$ sont disjoints. Si une hyperarête $e_u$ contenait à la fois $j$ et $j'$, alors $|e_u \cap e_v| \ge 2$, ce qui contredirait la linéarité de $H$.
...
-- Definition d'un hypergraphe lineaire dual
structure LinearHypergraph (H_V : Type*) [Fintype H_V] [DecidableEq H_V] where
  (edges : Finset (Finset H_V))
  (linear : forall e1 e2 \in edges, e1 != e2 -> (e1 \inter e2).card <= 1)

lemma efl_equivalence_dual (k : Nat) (sys : IntersectingCliqueSystem k) :
  exists H : LinearHypergraph (Fin k),
    -- La preuve constructive du Lemme 1 vient ici
    True := sorry

lemma dual_degree_bound (k : Nat) (H : LinearHypergraph (Fin k)) (v : Fin k) :
  -- Borne sur le degre d'un sommet dans l'hypergraphe
  (H.edges.filter (fun e => v \in e)).card <= k := sorry
