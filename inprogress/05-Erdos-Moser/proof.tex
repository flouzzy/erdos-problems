...
\begin{abstract}
Dans ce document, nous explorons la célèbre équation diophantienne d'Erdős-Moser : $\sum_{i=1}^{m-1} i^k = m^k$. Nous présentons une série de lemmes rigoureux analysant la parité de l'exposant $k$, les contraintes de divisibilité par des nombres premiers, et les comportements asymptotiques. Les démonstrations sont rédigées pas à pas, explicitement, afin de fournir un cadre formel complet traduisible dans un système de preuve mécanisée tel que Lean 4.
\end{abstract}
