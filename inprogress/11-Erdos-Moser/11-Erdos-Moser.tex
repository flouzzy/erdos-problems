...
\lstset{
    ...
    morekeywords={import, def, lemma, theorem, have, sorry, by, open, Prop, forall, exists, Nat}
}
...
We decompose the structural constraints of the Erd\H{o}s-Moser equation into three primary lemmata. We present the proofs completely explicitly, detailing every logical step and algebraic manipulation.
...
To guarantee the rigor of the structural proofs presented above, we detail the architectural blueprint for their verification using the Lean 4 proof assistant. The definitions establish the formal context, while the theorem signatures map directly to the informal lemmata.
