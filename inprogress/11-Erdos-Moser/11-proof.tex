...
lemma erdos_moser_faulhaber_bound (k m : Nat) (hk : k >= 2) (hm : m >= 2) (h_eq : ErdosMoserPredicate k m) :
  ...
  have h3 : (k + 1) * m^k < m^(k+1) := by
    ...
    -- Il s'agit d'une esquisse de preuve incomplete destinee a une autoformalisation future.
    have h_mul_div : ((m^(k+1)) / (k + 1)) * (k + 1) <= m^(k+1) := Nat.div_mul_le_self _ _
    have h2_mul : m^k * (k + 1) < ((m^(k+1)) / (k + 1)) * (k + 1) := by
      exact Nat.mul_lt_mul_of_pos_right h2 (by omega)
    rw [Nat.mul_comm (k + 1) _]
    exact lt_of_lt_of_le h2_mul h_mul_div
  have h4 : k + 1 < m := by
    -- Dividing by m^k
    -- Il s'agit d'une esquisse de preuve incomplete destinee a une autoformalisation future.
    have h_pow : m^(k+1) = m^k * m := by rw [Nat.pow_succ, Nat.mul_comm]
    rw [h_pow] at h3
    rw [Nat.mul_comm (k + 1) (m^k)] at h3
