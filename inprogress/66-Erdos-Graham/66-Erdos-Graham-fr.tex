...
\title{Vers le probl\`eme d'Erd\H{o}s-Graham sur les fractions \'egyptiennes}
...
\section{Contexte litt\'eraire et analogie}
Historiquement, les th\'eor\`emes de densit\'e comme le th\'eor\`eme de Szemer\'edi fournissent des garanties structurelles profondes pour des ensembles de densit\'e sup\'erieure strictement positive. Cependant, la densit\'e seule ne r\'egit pas le probl\`eme d'Erd\H{o}s-Graham, car on peut colorier des intervalles $I_k = [N_k, N_{k+1}-1]$ de sorte que la somme des inverses dans une seule couleur diverge, tout en exigeant que la structure locale force la somme \`a valoir exactement 1.

Nous \'etablissons une analogie avec le th\'eor\`eme r\'ecemment r\'esolu de Bloom (2021) qui d\'emontre la conjecture, prolongeant la perc\'ee de Croot en 2003 sur l'existence de sous-ensembles monochromatiques sommant \`a 1 pour des ensembles de densit\'e positive. L'approche ici reconstruit et d\'eveloppe les m\'ethodes de Fourier-analytiques et combinatoires (la m\'ethode du cercle) adapt\'ees aux ensembles discrets clairsem\'es.
...
\end{lemma}
\begin{proof}
Soit $C$ un sous-ensemble de $\mathbb{N}_{\geq 2}$ avec $\sum_{n \in C} 1/n = \infty$. Nous employons une m\'ethode de crible quantitative. Soit $\Psi(x, y)$ le nombre d'entiers $y$-friables jusqu'\`a $x$. D'apr\`es les r\'esultats classiques de Dickman et de Bruijn, la densit\'e des nombres $y$-friables est bien comprise. Pour un seuil soigneusement choisi $y = x^{1/u}$ o\`u $u$ est un grand param\`etre, l'intersection de $C$ avec l'ensemble des nombres friables doit conserver une somme divergente des inverses, sinon, les \'el\'ements non-friables domineraient, contredisant la distribution uniforme des grands facteurs premiers \`a travers les progressions arithm\'etiques. Nous omettons le crible complet ici, mais le principe est explicite : si nous restreignons $C$ aux \'el\'ements dont les facteurs premiers sont born\'es par $y$, la propri\'et\'e divergente persiste.
\end{proof}
...
\section{Conclusion}
Ce cadre isole pr\'ecis\'ement les \'etapes combinatoires n\'ecessaires pour aborder le probl\`eme d'Erd\H{o}s-Graham. En d\'ecomposant le d\'efi en lemmes v\'erifiables concernant la densit\'e et la friabilit\'e, nous ouvrons une voie claire pour la v\'erification formelle compl\`ete de la preuve.
