...
\label{lem:reduction}
Soit $f : \mathbb{N}^+ \to \{-1, 1\}$. S'il existe une constante $C > 0$ telle que pour tout $d \in \mathbb{N}^+$ et tout $N \in \mathbb{N}^+$, $\Delta(f, d, N) \leq C$, alors il existe une fonction complètement multiplicative $g : \mathbb{N}^+ \to \{-1, 1\}$ (c'est-à-dire $g(ab) = g(a)g(b)$ pour tous $a, b \in \mathbb{N}^+$) qui satisfait une propriété de discrépance bornée similaire, modulo des modifications mineures.
\end{lemma}
\begin{proof}
...
\end{equation}
Par le Lemme~\ref{lem:reduction}, cette hypothèse implique l'existence d'une fonction complètement multiplicative $g : \mathbb{N}^+ \to \{-1, 1\}$ héritant de propriétés de bornitude étroitement liées à $C$.
