...
\author{Charles EDOU NZE, ingenieur informatique - Mathematicien amateur}
\date{}
...
def DivisorLattice (N : Nat) : Finset Nat :=
  (Finset.range (N + 1)).filter (fun d => d > 0 /\ N % d = 0)

theorem audige_greedy_step (n : Nat) (hn : n >= 2) :
  exists d e f : Nat, d \in DivisorLattice (L_n n) /\ e \in DivisorLattice (L_n n) /\
  f \in DivisorLattice (L_n n) /\ d + e + f = T_n n hn := by
  sorry
...
theorem erdos_straus_conjecture_from_audige : forall n : Nat, n >= 2 -> ErdosStrausPredicate n := by
  ...
  rcases h_audige with \langle d, e, f, hd, he, hf, hsum\rangle
  unfold ErdosStrausPredicate
  use (L_n n)/d, (L_n n)/e, (L_n n)/f
  sorry
\end{lstlisting}
